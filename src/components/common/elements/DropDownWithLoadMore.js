...
const DropDownWithLoadMore = ({ field }) => {
  ...
  const isMulti=allProps.isMultiSelect
    ...
    if (isMulti) {
      setSelectedItems((prev) => {
        if (prev.includes(value)) {
          ...
          // onChange(filtered, 1, option);
          return filtered;
        } else {
          const updated = [...prev, value];
          // onChange(updated, 1, option);
          return updated;
        }
      });
    } else {
  ...
  const checkDefaultSelected = () => {
    if (!isMulti) {
      ...
      if (filterData.length > 0) {
        setSelectedItems(filterData[0][optionData.id]);
        setSelectedItemName(
  ...
  const getSelectedLabel = () => {
    if (isMulti && selectedItems?.length > 0) {
      return selectedItems
        .map((id) => actualOptData().find((item) => item[optionData.id] === id)?.[optionData.label])
        .join(", ");
    } else {
      const sel = actualOptData().find((item) => item[optionData.id] === selectedItems);
      return sel ? sel[optionData.label] : placeholder || `Select ${label}`;
    }
  };
  return (
        ...
        <Dropdown className={"w-100" + " d-inline-block  specialInnerSelect " + (readOnly && "labelInput")}
          onToggle={(isOpen) => {
            ...
            if (isOpen) {
              ...
              if (isMulti && selectedItems?.length > 0) {
                const selectedItemsOPt = actualOptData().filter((item) => selectedItems?.includes(item[optionData.id]))
                onChange(selectedItems, 1, selectedItemsOPt)
              }

            }
          }}
          autoClose={isMulti ? "outside" : true}>
            ...
            <div className="loadMoreOptions" ref={dropdownMenuRef}>
              ...
              {actualOptData()?.map((item, index) => (
                ...
                <Dropdown.Item
                  ...
                  active={isMulti ? selectedItems?.includes(item[optionData.id]) : selectedItems === item[optionData.id]}
...
DropDownWithLoadMore.propTypes = {
  field: PropTypes.shape({
    ...
    isMulti: PropTypes.bool, // NEW: multi-select prop
  }),
};

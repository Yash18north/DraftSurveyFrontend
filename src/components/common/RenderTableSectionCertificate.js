...
import { getChangeOnShipsValue, getLMSOperationActivity, getOperationActivityUrl, getVesselOperation } from "../../services/commonFunction";
...
const RenderAssignmentTableSection = ({
  ...
  const initialCustomData = {
    [sectionIndex]: {
      smpl_set_basisjson: [],
      smpl_set_smpljson: [],
      is_group_param: GetTenantDetails(1, 1, formData[0]?.jrf_is_petro) == "TPBPL" ? "Parameter" : "Parameter",
      is_group_param_name:
        GetTenantDetails(1, 1, formData[0]?.jrf_is_petro) == "TPBPL" ? "Parameter" : "Parameter",
      smpl_set_paramjson: "",
  ...
  const onCustomChangeHandler = (indexNo, name, value) => {
    ...
    if (name === "configured_smpl") {
      setCustomFormData((prevData) => {
        return {
          ...prevData,
          [indexNo]: {
            ...prevData[indexNo],
            ['isSampleCustomChange']: 1,
          },
        };
      });
    }
    ...
    if (res?.data.status === 200) {
      ...
      res.data.data?.jrf_sample_marks.forEach((opt) => {
        if (opt.composite_mark) {
          opt.sample_marks.forEach((opt1) => {
            newoptions.push({
              label: opt1.sample_mark,
              // value: opt.sample_mark,
              value: opt1.sample_mark + '--###TCRCOPS###--' + opt1.sample_id,
            });
          });
  ...
  useEffect(() => {
    ...
    // if(custTemp){
    //   custTemp.map((singleSampleSet) => {
    //     if (singleSampleSet?.includes("(comp)")) {
    //       const targetSampleMark = singleSampleSet?.replace("(comp)", "").trim();
    //       const sptargetSampleMark=targetSampleMark.split('--###TCRCOPS###--')
    //       const matchingComposite = sampleAllOptions.find(
    //         (item) => item.sample_id === sptargetSampleMark[1]
    //       );
    ...
    if (custTemp?.[custTemp.length - 1]?.includes("(comp)")) {
      //Commented for composite unique validation removed 19-03-2024 by sufiyan
      // setCustomFormData((prevData) => {
  ...
  // configCertStatus
  useEffect(() => {
    if (customFormData[2]?.isConfigLoaded) {
      getCertConfig();
    }

  }, [configCertStatus, customFormData[2]?.isConfigLoaded])
      ...
      if (res?.data?.status === 200) {
        ...
        if (filteredData) {
          ...
          filteredData?.cert_config_sampleids.map((opt) => {
            if (!newoptions.find((singleopt) => singleopt.value == opt.sample_mark + '--###TCRCOPS###--' + opt.sample_id)) {
              newoptions.push({
                label: opt.sample_mark,
                value: opt.sample_mark + '--###TCRCOPS###--' + opt.sample_id,
              });
            ...
            if (!selectedsamples.includes(opt.sample_mark + '--###TCRCOPS###--' + opt.sample_id)) {
              selectedsamples.push(opt.sample_mark + '--###TCRCOPS###--' + opt.sample_id)
            }
          })
          setSampleOptions(newoptions)
          ...
          setCustomFormData((prevFormData) => {
            return {
              ...
              2: {
                ...prevFormData[2],
                configured_smpl: selectedsamples,
                isSampleCustomChange: 0
              },
            };
          });
          ...
          let combinearray = []
          uniqueData.forEach((singleData) => {
            if (!combinearray.find((cbnparam) => cbnparam.param_id == singleData.param_id)) {
              let comonBasis = uniqueData.filter((bsParam) => {
                return bsParam.param_id == singleData.param_id
              })
              singleData.basisItems = comonBasis
              combinearray.push(singleData)

            }
          });

          combinearray.map((param, paramIndex) => {
            param.basisItems.map((basisCode, basisIndex) => {
              const key = "rpc_is_avg_" + paramIndex + "_" + basisIndex;
              const deckey = "rpc_after_decimal_value_" + paramIndex + "_" + basisIndex;
              setCustomFormData((prevData) => {
                return {
                  ...prevData,
                  0: {
                    ...prevData[0],
                    [key]: basisCode?.is_display ? "yes" : "No",
                    [deckey]: basisCode?.cert_config_rpc_after_decimal_value,
                  },
                  1: {
                    ...prevData[1],
                    [key]: basisCode?.is_display ? "yes" : "No",
    ...
    if (res.data.status === 200) {
      ...
      res.data.data?.forEach((singleData) => {
        singleData?.forEach((singleSetData) => {
          ...
          if (singleSetData.jila_set_paramjson.length > 0) {
            ...
            // });
            singleSetData.jila_set_paramjson?.forEach((parameterdata) => {
              parameterdata.basis?.forEach((basisItem) => {
                const alreadyExists = paramdata.some((existingParam) =>
                  existingParam.param_id === parameterdata.param_id &&
                  existingParam.basis?.some((b) => b.basis_id === basisItem.basis_id)
                );

                if (!alreadyExists) {
                  // If the same param_id exists, add the new basis to it
                  const sameParam = paramdata.find(
                    (p) => p.param_id === parameterdata.param_id
                  );

                  if (sameParam) {
                    // Check if basis array exists; if not, initialize
                    if (!Array.isArray(sameParam.basis)) {
                      sameParam.basis = [];
                    }
                    sameParam.basis.push(basisItem);
                  } else {
                    // If completely new param_id, push a new entry
                    paramdata.push({
                      ...parameterdata,
                      basis: [basisItem],
                    });
      ...
      combinedDetails.map((param, paramIndex) => {
        return param.basis.map((basisItem, basisIndex) => {
          ...
          if (customFormData[2]?.isSampleCustomChange) {
            const key = "rpc_is_avg_" + paramIndex + "_" + basisIndex;
  ...
  const updateConfiguredStatus = async (status, configID) => {
    try {
      ...
      if (res.data.status === 200) {
        ...
        const redirectUrl = getOperationActivityUrl(formData[0]?.fk_operationtypetid_code)
        navigate(
          ...
          "&ConfigID=" +
          encryptDataForURL(configID) + "&operationMode=" + encryptDataForURL(formData[0]?.fk_operationtypetid_code)
        );
      } else {
    ...
    try {
          ...
          return setwiseParam.map((param, paramIndex) => {
            return param.basis.map((basisItem, basisIndex) => {
              return {
                ...
                cert_config_rpc_after_decimal_value:
                  customFormData[0]?.["rpc_after_decimal_value_" + paramIndex + "_" + basisIndex] || 2,
      ...
      const structuredData = config_data.reduce((acc, item) => {
        ...
        acc[mark].push({
          ...
          cert_config_rpc_after_decimal_value: item.cert_config_rpc_after_decimal_value || 2,
            ...
            if (!isSave) {
              ...
              setFormData((prevData) => {
                return {
                  ...
                  1: {
                    ...prevData[1],
                    "cert_config_id": res.data.data.cert_config_id,
                  },
                };
              });
            }
          ...
          <table className="table table-white responsive borderless no-wrap align-middle renderTable JICertificate">
            <thead>
              <tr>
                ...
                <th>Display</th>
                {formData[1]?.rpc_is_wght_avg?.[0] === "Weighted Avg." && <th>Show digit after decimal</th>}
                <th>Wt. Avg</th>
              </tr>
            </thead>
            <tbody>
                  ...
                  return (
                    param?.basis?.map((basisItem, basisIndex) => (
                      <tr>
                        ...
                        {
                          // formData[1]?.rpc_is_wght_avg?.[0] === "Weighted Avg." &&
                          <td>
                            <RenderFields
                              field={{
                                width: 2,
                                label: "",
                                name: "rpc_after_decimal_value_" + paramIndex + "_" + basisIndex,
                                type: "number",
                                fieldWidth: "100",
                                isOptionLabelNotShow: true,
                                readOnly: opsCertiView === "view"
          ...
          <button
            onClick={() => {
              ...
              !getLMSOperationActivity().includes(OperationType) ? navigate(
                `${redirectUrl}${encryptDataForURL(EditRecordId)}`
              ) : navigate(
                `${redirectUrl}confirugation-certificate-list/${encryptDataForURL(EditRecordId)}/${encryptDataForURL(EditSubRecordId)}?OperationType=${encryptDataForURL(OperationType)}` + "&operationMode=" + encryptDataForURL(formData[0]?.fk_operationtypetid_code)
              )
            }
            }
            className="cancelBtn"
